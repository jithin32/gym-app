          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100 text-left">
                ...
                <th className="px-4 py-3 font-semibold text-gray-600">Mode</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-4 py-3 text-gray-600">{p.due_date ? new Date(p.due_date).toLocaleDateString() : '—'}</td>
                  <td className="px-4 py-3 text-gray-600">{p.paid_date ? new Date(p.paid_date).toLocaleDateString() : '—'}</td>
                  <td className="px-4 py-3 text-gray-600 capitalize">{p.payment_mode ?? '—'}</td>
